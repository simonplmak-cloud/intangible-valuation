"""MCP tool wrappers for intangible valuation functions.

Each tool wraps a library function with JSON serialization and error handling.
"""
...
def _safe_json(obj: Any) -> str:
    """Serialize to JSON, handling Pydantic models and custom objects."""
...
def present_value(future_value: float, discount_rate: float, periods: int) -> str:
    ...
    """
    return _run(
        _present_value, future_value=future_value,
        discount_rate=discount_rate, periods=periods,
    )
...
def future_value(present_value: float, discount_rate: float, periods: int) -> str:
    ...
    """
    return _run(
        _future_value, present_value=present_value,
        discount_rate=discount_rate, periods=periods,
    )
...
def growing_annuity_pv(
    payment: float, discount_rate: float,
    growth_rate: float, periods: int,
) -> str:
    ...
    """
    return _run(
        _growing_annuity_pv, payment=payment,
...
def terminal_value(
    final_year_cashflow: float, perpetual_growth_rate: float,
    discount_rate: float, method: str = "gordon_growth",
    ...
    """
    return _run(
        _terminal_value, final_year_cashflow=final_year_cashflow,
        perpetual_growth_rate=perpetual_growth_rate,
        discount_rate=discount_rate, method=method,
...
def build_up_discount_rate(
    ...
    size_premium: float = 0.0, industry_risk_premium: float = 0.0,
    ...
    """
    return _run(
        _build_up_discount_rate, risk_free_rate=risk_free_rate,
...
def capm_discount_rate(
    ...
    """
    return _run(
        _capm_discount_rate, risk_free_rate=risk_free_rate,
        beta=beta, market_return=market_return,
    )


def wacc(
    equity_value: float, debt_value: float, cost_of_equity: float,
    ...
    """
    return _run(
        _wacc, equity_value=equity_value, debt_value=debt_value,
...
def tax_amortization_benefit(
    discount_rate: float, useful_life: int,
    tax_rate: float, asset_value: float,
) -> str:
    ...
    """
    return _run(
        _tax_amortization_benefit, discount_rate=discount_rate,
        useful_life=useful_life, tax_rate=tax_rate,
...
def control_premium(minority_price: float, control_price: float) -> str:
    ...
    """
    return _run(
        _control_premium, minority_price=minority_price,
        control_price=control_price,
    )


def dlom_finnerty(
    restricted_period: float, volatility: float,
    ...
    """
    return _run(
        _dlom_finnerty, restricted_period=restricted_period,
        volatility=volatility, risk_free_rate=risk_free_rate,
    )


def currency_adjusted_discount_rate(
    base_rate: float, currency_risk_premium: float = 0.0,
    ...
    """
    return _run(
        _currency_adjusted_discount_rate, base_rate=base_rate,
...
def reproduction_cost(
    ...
    """
    return _run(
        _reproduction_cost, development_costs=development_costs,
...
def replacement_cost(
    ...
    """
    return _run(
        _replacement_cost, current_cost=current_cost,
        obsolescence_factors=obsolescence_factors,
    )


def market_approach_comparables(
    comparables: list, subject_revenue: float,
    ...
    """
    return _run(
        _market_approach_comparables, comparables=comparables,
        subject_revenue=subject_revenue, adjustments=adjustments,
    )


def royalty_capitalization(
    revenue: float, royalty_rate: float,
    discount_rate: float,
) -> str:
    ...
    """
    return _run(
        _royalty_capitalization, revenue=revenue,
        royalty_rate=royalty_rate, discount_rate=discount_rate,
    )
...
def relief_from_royalty(
    revenue_projections: list, royalty_rate: float,
    discount_rate: float, tax_rate: float,
    useful_life: int, tab_enabled: bool = True,
) -> str:
    ...
    """
    return _run(
        _relief_from_royalty, revenue_projections=revenue_projections,
        royalty_rate=royalty_rate, discount_rate=discount_rate,
        tax_rate=tax_rate, useful_life=useful_life,
        tab_enabled=tab_enabled,
    )


def mpeem(
    cash_flow_projections: list, contributory_asset_charges: list,
    discount_rate: float, tax_rate: float, tab_enabled: bool = True,
) -> str:
    ...
    """
    return _run(
        _mpeem, cash_flow_projections=cash_flow_projections,
...
def single_period_excess_earnings(
    normalized_earnings: float, contributory_asset_charges: list,
    ...
    """
    return _run(
        _single_period_excess_earnings,
...
def incremental_cashflow(
    cash_flows_with: list, cash_flows_without: list,
    discount_rate: float,
) -> str:
    ...
    """
    return _run(
        _incremental_cashflow, cash_flows_with=cash_flows_with,
...
def patent_valuation(
    remaining_life: int, cash_flow_projections: list,
    probability_of_success: float, discount_rate: float,
    comparable_license_rates: list | None = None,
) -> str:
    ...
    """
    return _run(
        _patent_valuation, remaining_life=remaining_life,
...
def trademark_valuation(
    revenue: float, profit_margin: float, brand_strength_index: float,
    discount_rate: float, useful_life: int,
    ...
    """
    return _run(
        _trademark_valuation, revenue=revenue,
        profit_margin=profit_margin,
        brand_strength_index=brand_strength_index,
        discount_rate=discount_rate, useful_life=useful_life,
        method=method,
    )


def copyright_valuation(
    projected_revenue: float, useful_life: int,
    discount_rate: float, royalty_rate: float,
) -> str:
    ...
    """
    return _run(
        _copyright_valuation, projected_revenue=projected_revenue,
        useful_life=useful_life, discount_rate=discount_rate,
        royalty_rate=royalty_rate,
    )


def trade_secret_valuation(
    development_cost: float, economic_life: int,
    competitive_advantage_period: int, discount_rate: float,
    ...
    """
    return _run(
        _trade_secret_valuation, development_cost=development_cost,
...
def developed_technology_valuation(
    rd_costs: float, life_cycle_stage: str,
    competitive_advantage: int, discount_rate: float,
    cash_flow_projections: list,
) -> str:
    ...
    """
    return _run(
        _developed_technology_valuation, rd_costs=rd_costs,
...
def software_valuation(
    development_cost: float, maintenance_cost: float,
    user_base: int, revenue_model: dict,
    useful_life: int, discount_rate: float,
) -> str:
    ...
    """
    return _run(
        _software_valuation, development_cost=development_cost,
        maintenance_cost=maintenance_cost, user_base=user_base,
        revenue_model=revenue_model, useful_life=useful_life,
        discount_rate=discount_rate,
    )


def data_asset_valuation(
    acquisition_cost: float, quality_score: float,
    revenue_contribution: float, useful_life: int,
    discount_rate: float,
) -> str:
    ...
    """
    return _run(
        _data_asset_valuation, acquisition_cost=acquisition_cost,
...
def platform_valuation(
    network_size: int, network_effects_coefficient: float,
    revenue_per_user: float, growth_rate: float,
    discount_rate: float,
) -> str:
    ...
    """
    return _run(
        _platform_valuation, network_size=network_size,
        network_effects_coefficient=network_effects_coefficient,
        revenue_per_user=revenue_per_user, growth_rate=growth_rate,
        discount_rate=discount_rate,
    )


def customer_relationship_valuation(
    customer_count: int, avg_revenue_per_customer: float,
    retention_rate: float, profit_margin: float,
    discount_rate: float, projection_period: int,
) -> str:
    ...
    """
    return _run(
        _customer_relationship_valuation,
...
def distribution_network_valuation(
    channel_count: int, revenue_per_channel: float,
    channel_margin: float, useful_life: int,
    discount_rate: float,
) -> str:
    ...
    """
    return _run(
        _distribution_network_valuation, channel_count=channel_count,
...
def non_compete_valuation(
    protected_revenue: float, profit_margin: float,
    term: int, enforcement_probability: float,
    discount_rate: float,
) -> str:
    ...
    """
    return _run(
        _non_compete_valuation, protected_revenue=protected_revenue,
        profit_margin=profit_margin, term=term,
...
def assembled_workforce_valuation(
    employee_count: int, avg_replacement_cost: float,
    training_cost: float, productivity_factor: float,
    ...
    """
    return _run(
        _assembled_workforce_valuation, employee_count=employee_count,
...
def key_person_value(
    revenue_contribution: float, replacement_cost: float,
    departure_probability: float, discount_rate: float,
) -> str:
    ...
    """
    return _run(
        _key_person_value, revenue_contribution=revenue_contribution,
...
def goodwill(
    ...
    """
    return _run(
        _goodwill, purchase_price=purchase_price,
        fair_value_net_identifiable_assets=fair_value_net_identifiable_assets,
    )


def purchase_price_allocation(
    purchase_price: float, tangible_assets_fv: float,
    identified_intangibles: list, liabilities_fv: float = 0,
) -> str:
    ...
    """
    return _run(
        _purchase_price_allocation, purchase_price=purchase_price,
...
def goodwill_impairment_test(
    carrying_value: float, fair_value: float,
    reporting_unit: str = "", standard: str = "ASC350",
) -> str:
    ...
    """
    return _run(
        _goodwill_impairment_test, carrying_value=carrying_value,
        fair_value=fair_value, reporting_unit=reporting_unit,
        standard=standard,
    )


def intangible_impairment_test(
    carrying_value: float, fair_value: float | None = None,
    recoverable_amount: float | None = None, standard: str = "ASC350",
) -> str:
    ...
    """
    return _run(
        _intangible_impairment_test, carrying_value=carrying_value,
...
def royalty_rate_benchmark(
    ip_type: str, industry: str,
    comparable_database: list | None = None,
) -> str:
    ...
    """
    return _run(
        _royalty_rate_benchmark, ip_type=ip_type, industry=industry,
...
def adjust_royalty_rate(base_rate: float, adjustment_factors: dict) -> str:
    ...
    """
    return _run(
        _adjust_royalty_rate, base_rate=base_rate,
...
def cup_transfer_price(
    controlled_price: float, uncontrolled_prices: list,
) -> str:
    ...
    """
    return _run(
        _cup_transfer_price, controlled_price=controlled_price,
        uncontrolled_prices=uncontrolled_prices,
    )


def patent_infringement_damages(
    lost_profits_or_royalty: float, infringement_period: int,
    discount_rate: float, prejudgment_interest_rate: float,
) -> str:
...
def monte_carlo_valuation(
    input_distributions: list, iterations: int = 10000,
    seed: int | None = None,
) -> str:
    """Run Monte Carlo simulation for valuation uncertainty analysis.

    Note: This tool uses a simplified interface. For full flexibility,
    use the Python API.
    ...
    def _simple_valuation(**params: float) -> float:
        """Simple sum-based valuation for MC demo."""
        return sum(
    ...
    return _run(
        _monte_carlo_valuation, valuation_fn=_simple_valuation,
...
def monte_carlo_sensitivity(
    base_params: dict, distributions: dict,
    iterations: int = 10000, seed: int | None = None,
) -> str:
    ...
    return _run(
        _monte_carlo_sensitivity, valuation_fn=_simple_valuation,
...
def sensitivity_analysis(
    function_name: str, parameter_name: str,
    parameter_range: list, fixed_parameters: dict,
) -> str:
    """Perform one-at-a-time sensitivity analysis on a valuation function.

    Supported functions: present_value, future_value, annuity_pv,
    perpetuity_pv, growing_annuity_pv, terminal_value,
    build_up_discount_rate, capm_discount_rate, wacc.

    Book Reference: Appendix A, Section A.3
    """
    return _run(
        _sensitivity_analysis, function_name=function_name,
...
def estimate_useful_life(
    asset_type: str, legal_life: float | None = None,
    economic_factors: dict | None = None,
    ...
    """
    return _run(
        _estimate_useful_life, asset_type=asset_type,
